...
class LinksPresenter extends BasePresenter {
    ...
    function createComponentEditForm() {
        ...
        $form->setDefaults(array(
            ...
            "category" => $this->template->link->categories_id,
    ...
    function editFormSucceeded(\Nette\Forms\BootstrapUIForm $form) {
        ...
        if (file_exists(APP_DIR . '/images/links/' . $uid . ".jpg") && is_uploaded_file($_FILES['the_file']['tmp_name'])) {
            \App\Model\IO::remove(APP_DIR . '/images/links/' . $uid . ".jpg");
        }
        \App\Model\IO::upload(APP_DIR . '/images/links', $uid . ".jpg", 0644);

        $this->redirect(":Admin:Links:detail", array("id" => $form->values->id));
    }

    function handleDeleteImage($id) {
        \App\Model\IO::remove(APP_DIR . '/images/links/' . \Nette\Utils\Strings::padLeft($id, 6, '0') . '.jpg');
    ...
    function handleDeleteCategory($id) {
        ...
        $this->database->table("links")->where(array("categories_id" => $id))->update(array("categories_id" => 1));
    ...
    public function renderDefault() {
        $this->template->categoryId = $this->template->settings['categories:id:link'];

        if ($this->getParameter("id")) {
            $this->template->links = $this->database->table("links")
                    ->where("categories_id", $this->getParameter("id"))
                    ->order("title");
        } else {
    ...
    public function renderCategories() {
        $this->template->categories = $this->database->table("categories")->order("category");
    }
