...
class ProductFilter
{
    ...
    function assemble()
    {
        if ($this->category != FALSE) {
            $columns[":store_category.pages_id"] = $this->category;
        }
        ...
        $connect = $this->database->table("store")
            ->select("pages.id, store.id, pages.title AS title, pages.slug AS slugtitle, pages.preview, 
            pages.date_created, pages.document, pages.date_published, :stock.amount, "
                . "SUM(:stock.amount) AS sumstock, price, "
                . ":params.param_id, :params.paramvalue, store.sorted");
